...
const nav = [
  ...
  { to: "/entrada", label: "Marcar Entrada", icon: Pencil },
...
export default function Sidebar() {
  ...
  const visibleNav = user?.rol === "ADMIN"
    ? nav
    : [
  ...
  return (
    <aside className="w-64 min-h-screen bg-blue-900 text-white flex flex-col">
      ...
      <div className="p-4 border-t border-blue-800">
        <div className="text-xs text-blue-300 mb-1">{user?.nombre}</div>
        <div className="text-[11px] text-blue-400 mb-3">
          {user?.rol === "ADMIN" ? "Administrador" : "Usuario estandar"}
        </div>
        <button
          onClick={logout}
